...
export const introSchema = z.object({
  accept_terms_and_conditions: z.boolean().refine((value) => value === true, {
    error: "You need to check this option to Access the Scheme",
  }),
});
...
export const submitSchema = z.object({
  agree_topass_info_to_banking_partner: z.boolean().refine((value) => value === true, {
    error: "You need to check this option to submit the application",
  }),
});
...
export const declineApplicationSchema = z.object({
  ...
  path: ["decline_all"],
  error: "You need to check at least one option to Decline the Scheme",
});
...
export const repaymentTermsSchema = z.object({
  repayment_years: z.coerce
    .number({
      error: (issue) => (issue.input === undefined ? "Years is required" : "Years must be a number"),
    })
  ...
  payment_start_date: z
    ...
    .refine((value) => isDateAfterCurrentDate(value), {
      error: "Payment start date must be after current date",
    }),
});
...
export const approveSchema = z.object({
  ...
  disbursed_final_amount: z.coerce
    .number({
      error: (issue) =>
        issue.input === undefined ? "Disbursed final amount is required" : "Disbursed final amount must be a number",
    })
